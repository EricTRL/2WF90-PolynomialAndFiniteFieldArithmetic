...
public class Division {
    ...
    public static Polynomial modulo(Polynomial x, int m) {
        return olddivide(x, new Polynomial(m)).r;
    }
     ...
     */
    public static QuoRem olddivide(Polynomial x, Polynomial y) {
        ...
        for(int i = 0; i < k - l + 1; i++) { //LINE 3 AlGO
            q.addLast((r.get(i)*b+r.get(i+1))/y.getLeadingCoefficient()); //LINE 4 ALGO
        ...
        return new QuoRem(Arithmetic.removeLeadingZeros(qPoly), Arithmetic.removeLeadingZeros(rPoly)); //LINE 18 ALGO
    }   
